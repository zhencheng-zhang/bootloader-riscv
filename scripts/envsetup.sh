...
RV_ZSBL_CONF_INI=$RV_FIRMWARE_INSTALL_DIR/conf.ini
...
function show_rv_functions()
{
	...
	echo "build_rv_conf_ini_tmpl			-create a conf.ini template"
	...
	echo "build_rv_firmware                 -build firmware(zsbl,sbi,u-boot,grub2,kernel,uroot)"
	...
	echo "clean_rv_firmware                 -clean firmware(zsbl,sbi,u-boot,grub,kernel,uroot)"
...
function build_rv_conf_ini_tmpl()
{
    echo "[sophgo-config]" > $RV_ZSBL_CONF_INI
    echo "" >> $RV_ZSBL_CONF_INI
    echo "[devicetree]" >> $RV_ZSBL_CONF_INI
    echo "name = mango-sophgo-x8evb.dtb" >> $RV_ZSBL_CONF_INI
    echo "; addr = 0x20000000" >> $RV_ZSBL_CONF_INI
    echo "" >> $RV_ZSBL_CONF_INI
    echo "[mac-address]" >> $RV_ZSBL_CONF_INI
    echo "mac0 = 0xAAAAAAAAAAAA" >> $RV_ZSBL_CONF_INI
    echo "mac1 = 0xAAAAAAAAAAAA" >> $RV_ZSBL_CONF_INI
    echo "" >> $RV_ZSBL_CONF_INI
    echo "[kernel]" >> $RV_ZSBL_CONF_INI
    echo "; name = riscv64_Image" >> $RV_ZSBL_CONF_INI
    echo "name = u-boot.bin" >> $RV_ZSBL_CONF_INI
    echo "; addr = 0x02000000" >> $RV_ZSBL_CONF_INI
    echo "" >> $RV_ZSBL_CONF_INI
    echo "[eof]" >> $RV_ZSBL_CONF_INI
}
...
function build_rv_ubuntu_image()
{
    ...
    sudo cp $RV_FIRMWARE_INSTALL_DIR/conf.ini $RV_OUTPUT_DIR/efi/riscv64
...
function build_rv_firmware_package()
{
	...
	cp conf.ini firmware/riscv64
	cp riscv64_Image firmware/riscv64
	cp *.dtb firmware/riscv64
	cp initrd.img firmware/riscv64
	cp fw_jump.bin firmware/riscv64
	sudo cp u-boot.bin firmware/riscv64
	sudo cp grubriscv64.efi firmware/
	sudo cp grub.cfg firmware/
    touch firmware/BOOT
